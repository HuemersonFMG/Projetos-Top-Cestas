...
async function executarComRetrySankhya(fn) {
  try {
    const cookie = await login();
    return await fn(cookie);
  } catch (err) {
    console.warn('⚠️ Falha na primeira tentativa Sankhya. Renovando login...', err.message);

    limparLoginSankhya();

    const novoCookie = await login();
    return await fn(novoCookie);
  }
}
  ...
  switch (st) {
    case 'AGUARDANDO ROTA':
      return {
        ...
        statusOriginal: 'AGUARDANDO ROTA',
        classeStatus: 'status-aguardando-rota'
      };

    case 'ENTREGANDO':
      return {
        status: 'Entregando',
        statusOriginal: 'ENTREGANDO',
        classeStatus: 'status-entregando'
      };

    case 'NOTA EMITIDA':
      return {
        status: 'Nota Emitida',
        statusOriginal: 'NOTA EMITIDA',
        classeStatus: 'status-nota-emitida'
      };

    case 'ENTREGUE':
      return {
        status: 'Entregue',
        statusOriginal: 'ENTREGUE',
    ...
    default:
      return {
        status: st || '',
        statusOriginal: st || '',
        classeStatus: 'status-indefinido'
      };
  }
}
...
function montarLinks(filtrados, baseUrl) {
  return filtrados.map(r => {
    const infoStatus = normalizarStatusEntrega(r.ST_ENTREGAS?.$ || '');
    ...
    return {
      ...
      ST_ENTREGAS: infoStatus.statusOriginal,
      classeStatus: infoStatus.classeStatus,
...
app.post('/api/gerar-links', requireAdminApi, async (req, res) => {
  try {
    ...
    const lista = await executarComRetrySankhya(async (cookie) => {
      return await carregarViewLight(cookie);
    });
    ...
    const filtrados = lista.filter(r => {
      ...
      switch (tipoFiltro) {
        ...
        case 'DATA': {
          const rawDtFatur = String(r.DTFATUR?.$ || '').trim();
          ...
          if (/^\d{2}\/\d{2}\/\d{4}/.test(rawDtFatur)) {
            const [parteData] = rawDtFatur.split(' ');
            const [dia, mes, ano] = parteData.split('/');
            dataFaturamento = `${ano}-${mes}-${dia}`;
          } else if (/^\d{4}-\d{2}-\d{2}/.test(rawDtFatur)) {
            dataFaturamento = rawDtFatur.substring(0, 10);
          }

          return dataFaturamento === data;
        }
