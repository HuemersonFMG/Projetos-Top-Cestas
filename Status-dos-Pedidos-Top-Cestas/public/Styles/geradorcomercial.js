...
function getStatusPedido(item) {
  return String(
    extrairValor(item?.status) ||
    extrairValor(item?.ST_ENTREGAS) ||
    extrairValor(item?.statusOriginal) ||
    ''
  ).trim();
}

function getClasseStatusPedido(item) {
  return String(
    extrairValor(item?.classeStatus) ||
    ''
  ).trim();
}

function montarTagStatusPedido(item) {
  const status = getStatusPedido(item);
  let classe = getClasseStatusPedido(item);
  ...
  if (!classe) {
    switch (status.toUpperCase()) {
      case 'AGUARDANDO ROTA':
        classe = 'status-aguardando-rota';
        break;

      case 'ENTREGANDO':
        classe = 'status-entregando';
        break;

      case 'NOTA EMITIDA':
        classe = 'status-nota-emitida';
        break;
      ...
      default:
        classe = 'status-indefinido';
        break;
    }
  }

  return `<span class="status-tag ${classe}">${status}</span>`;
}
...
async function gerar() {
  ...
  try {
    ...
    dataRes.links.forEach(l => {
      ...
      const tagStatus = montarTagStatusPedido(l);
      ...
      el.innerHTML = `
        ...
        ${numNota ? `<br><b>NF-e:</b> ${numNota}` : ''}

        <br>
        <b>Cliente:</b>
        ${nomeCliente}

        ${periodo ? `<br><b>Período PAP:</b> ${periodo}` : ''}
